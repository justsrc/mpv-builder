...
def resolve_lib_path(objfile, lib, rapths):
    ...
    if lib.startswith("@rpath/"):
        ...
        libname = lib[len("@loader_path/") :]
        candidate = os.path.normpath(os.path.join(os.path.dirname(objfile), libname))
        if os.path.exists(candidate):
            return candidate
    ...
    if os.path.isdir(opt_dir):
        search_dirs.extend(
            ...
            if os.path.isdir(os.path.join(opt_dir, formula, "lib"))
        )
...
def process_vulkan_loader(binary, loader_name, loader_relative_folder, library_node):
    ...
    loader_system_search_folders = [
        ...
        os.path.join(
            get_homebrew_prefix(), "share", loader_relative_folder
        ),  # old location
    ]
